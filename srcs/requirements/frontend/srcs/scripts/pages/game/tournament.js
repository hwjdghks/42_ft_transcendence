function GameTournamentPage() {
  ...
  if (!options || !usernames) {
    ...
    // 데이터가 정상적으로 입력된 경우 토너먼트인 상황을 알리기 위해 세션 변수로 저장
  ...
  nextButton.addEventListener('click', () => handleNextButtonClick(matches, container));
  container.appendChild(nextButton);

  return container;
}

function handleNextButtonClick(matches, container) {
...
function renderBracket(matchData, parent) {  
  // 기존 내용 초기화 (bracket div 내부만)
  const divBracket = parent.querySelector('#bracket');
  if (divBracket) {
  ...
  matchData.forEach((match, index) => {
    ...
    divBracket.appendChild(matchCard);
  });

  parent.appendChild(divBracket);
}
